...
_stellar_blade_export_log = []


def reset_stellar_blade_export_log():
    _stellar_blade_export_log.clear()


def get_stellar_blade_export_log():
    return "\n".join(_stellar_blade_export_log)


def stellar_blade_log(settings, message=""):
    print(message)
    if getattr(settings, "stellar_blade_show_log", False):
        _stellar_blade_export_log.append(str(message))
        try:
    ...
    for bo_obj in bones:
        ...
        if bo_obj.name in skeleton_json[-1]["ReferenceSkeleton"]["FinalNameToIndexMap"]:
            ...
            if np.dot(scaleShould, scaleIs) < 0: # use dot product to check if vectors are oriented in opposite directions
                should_flip = True
                stellar_blade_log(scene_data.settings, bo_obj.name)
                if bo_obj in bones_actually_flipped.keys() and bones_actually_flipped[bo_obj]:
                    stellar_blade_log(scene_data.settings, "    This bone was parent-flipped")
                stellar_blade_log(scene_data.settings, f"    Scale should {scaleShould}")
                stellar_blade_log(scene_data.settings, f"    Scale is {scaleIs}")
                stellar_blade_log(scene_data.settings, f"    Dot Product {np.dot(scaleShould, scaleIs)}")
...
def save_single(operator, scene, depsgraph, filepath="",
                ...
                stellar_blade_show_log=True,
                stellar_blade_fix=True,
